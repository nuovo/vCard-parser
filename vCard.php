 ...
 * @version 0.4.8
 * 
 * @internal use get_called_class() for new vcard instances
 * this way you can totally extend vCard
 * 
 * Retro support for get_called_class(): https://gist.github.com/mirkolofio/5019277
 * Extension example: https://gist.github.com/mirkolofio/5019249
*/
	...
	public function __construct($Path = false, $RawData = false, array $Options = null)
	{
		...
		if ($this -> Mode == self::MODE_MULTIPLE)
		{
			...
			foreach ($this -> RawData as $SinglevCardRawData)
			{
				...
				$this -> Data[] = new $instanceName(false, $SinglevCardRawData);
			}
		}
		else
		{
			...
			foreach ($Lines as $Line)
			{
				...
				{
					$instanceName = get_called_class ();
					$Value = new $instanceName(false, str_replace('-wrap-', "\n", $Value));
